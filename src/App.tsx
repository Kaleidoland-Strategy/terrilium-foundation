...
export default function App() {
  ...
  return (
    <div className="vanta-edition min-h-screen bg-[#0a0a0a] text-white font-['Plus_Jakarta_Sans',sans-serif]">
      ...
      <header className="fixed top-0 w-full z-50 bg-[#0a0a0a]/80 backdrop-blur-md border-b border-white/5">
        <nav className="max-w-7xl mx-auto px-6 h-20 flex items-center justify-between">
          <a href="/" className="flex items-center gap-3 group">
            <img src="/logo.png" alt="Terrilium" className="h-10 w-auto transition-transform group-hover:scale-110" />
            <span className="text-xl font-black tracking-tighter text-white group-hover:text-[#D4AF37] transition-colors uppercase">TERRILIUM</span>
          </a>
          
          <div className="flex items-center gap-4 md:gap-8">
            <ul className="hidden md:flex items-center gap-8 text-[10px] font-bold tracking-[0.2em] uppercase">
              ...
              <li><button onClick={() => openModal('don')} className="bg-[#D4AF37] text-black px-6 py-2 rounded-sm font-black hover:bg-white transition-all shadow-[0_0_20px_rgba(212,175,55,0.3)]">{t('nav-support')}</button></li>
            </ul>

            <div className="flex items-center gap-6">
              <a href="https://www.instagram.com/terrilium.heritage/" target="_blank" rel="noopener noreferrer" className="text-white/10 hover:text-[#D4AF37] transition-all hover:scale-110" title="Instagram">
                <Instagram size={14} />
              </a>

              <div className="relative">
                <button onClick={() => setIsLangMenuOpen(!isLangMenuOpen)} className="flex items-center gap-2 text-[10px] font-bold border border-white/10 px-3 py-1.5 rounded-sm uppercase tracking-widest hover:border-[#D4AF37]/50 transition-colors">
                  <Globe size={12} className="text-[#D4AF37]" /> {lang} <ChevronDown size={10} className={`transition-transform ${isLangMenuOpen ? 'rotate-180' : ''}`} />
                </button>
                <AnimatePresence>
                  {isLangMenuOpen && (
                    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 10 }} className="absolute right-0 mt-2 bg-[#111] border border-white/10 rounded-sm overflow-hidden flex flex-col min-w-[140px] shadow-2xl z-[60]">
                      <button onClick={() => { setLang('fr'); setIsLangMenuOpen(false); }} className="px-4 py-2.5 text-left hover:bg-white/5 text-[10px] font-bold tracking-widest uppercase">Français</button>
                      <button onClick={() => { setLang('en'); setIsLangMenuOpen(false); }} className="px-4 py-2.5 text-left hover:bg-white/5 text-[10px] font-bold tracking-widest uppercase">English</button>
                    </motion.div>
                  )}
                </AnimatePresence>
              </div>
      ...
      <main className="pt-20">
        <section className="min-h-[85vh] flex items-center px-6 relative overflow-hidden">
          ...
          <div className="max-w-5xl mx-auto text-center relative z-10">
            <motion.p initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="text-[#D4AF37] uppercase tracking-[0.4em] text-[10px] font-black mb-8">{t('hero-eyebrow')}</motion.p>
            <motion.h1 initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="text-5xl md:text-9xl font-black leading-[1] mb-10 tracking-tighter uppercase" dangerouslySetInnerHTML={{ __html: t('hero-title') }} />
            <motion.p initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="text-lg md:text-xl text-white/50 max-w-2xl mx-auto leading-relaxed font-light" dangerouslySetInnerHTML={{ __html: t('hero-lead') }} />
            <div className="w-px h-32 bg-gradient-to-b from-[#D4AF37] to-transparent mx-auto mt-20" />
          </div>
        </section>
        ...
        <section id="manifeste" className="py-40 px-6 bg-white/[0.02]">
          <div className="max-w-7xl mx-auto grid md:grid-cols-2 gap-24 items-end">
            <div>
              <h2 className="text-[#D4AF37] font-bold text-[10px] tracking-[0.3em] uppercase mb-16">{t('tag-01')}</h2>
            ...
            <div className="flex flex-col">
              ...
              <a href="#soutenir" className="text-[#D4AF37] font-black uppercase tracking-[0.2em] text-xs flex items-center gap-4 group">
        ...
        <section id="actions" className="py-40 px-6">
          <div className="max-w-7xl mx-auto">
            <h2 className="text-[#D4AF37] font-bold text-[10px] tracking-[0.3em] uppercase mb-24">{t('tag-02')}</h2>
            <div className="grid md:grid-cols-3 gap-8">
              {[1, 2, 3].map((num) => (
                <div key={num} className="group p-12 border border-white/5 hover:border-[#D4AF37]/20 transition-all bg-white/[0.01] relative overflow-hidden">
                  <div className="absolute top-0 left-0 w-1 h-0 group-hover:h-full bg-[#D4AF37] transition-all duration-500" />
        ...
        <section id="soutenir" className="py-40 px-6 bg-[#D4AF37]/5">
          <div className="max-w-4xl mx-auto text-center">
            <h2 className="text-[#D4AF37] font-bold text-[10px] tracking-[0.3em] uppercase mb-16">{t('tag-03')}</h2>
            ...
            <div className="grid md:grid-cols-2 gap-8 mb-24">
              <div className="p-12 bg-[#0a0a0a] border border-white/5 text-left group hover:border-[#D4AF37]/30 transition-colors">
                <h3 className="text-2xl font-extrabold mb-6 uppercase tracking-tighter text-[#D4AF37]">{t('card-don-h3')}</h3>
                <p className="text-white/40 text-sm mb-10 font-light leading-relaxed">{t('card-don-p')}</p>
                <button onClick={() => openModal('don')} className="w-full py-5 bg-[#D4AF37] text-black font-black uppercase tracking-[0.2em] text-[10px] hover:bg-white transition-all">{t('btn-don')}</button>
              </div>
              <div className="p-12 bg-[#0a0a0a] border border-white/5 text-left group hover:border-[#D4AF37]/30 transition-colors">
                <h3 className="text-2xl font-extrabold mb-6 uppercase tracking-tighter text-[#D4AF37]">{t('card-adh-h3')}</h3>
                <p className="text-white/40 text-sm mb-10 font-light leading-relaxed">{t('card-adh-p')}</p>
                <a href="#adhesion-area" className="block w-full py-5 border border-[#D4AF37] text-[#D4AF37] text-center font-black uppercase tracking-[0.2em] text-[10px] hover:bg-[#D4AF37] hover:text-black transition-all">{t('btn-adh')}</a>
              </div>
            </div>
            ...
            <div className="flex-1 overflow-y-auto">
              {activeModal === 'don' ? (
                ...
                <div className="max-w-3xl mx-auto py-24 px-6">
                  <h2 className="text-4xl font-extrabold mb-12 tracking-tighter uppercase">{activeModal === 'legal' ? 'Mentions Légales' : 'Confidentialité'}</h2>
        ...
        {showCookieBanner && (
          <motion.div initial={{ y: 100, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: 100, opacity: 0 }} className="fixed bottom-8 right-8 md:w-[400px] z-[200] bg-[#111] border border-white/10 p-8 rounded-sm shadow-2xl">
            ...
            <div className="flex gap-4">
              <button onClick={() => handleCookieAction(true)} className="flex-1 py-4 bg-white text-black text-[10px] font-black uppercase tracking-widest rounded-sm hover:bg-[#D4AF37] transition-all">{t('cookie-accept')}</button>
